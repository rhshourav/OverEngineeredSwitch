...
void touchMe(){
  ...
  while (initCount == true && touchLoopCount <= 20){
    uint16_t w, h;
...
void displayTask(void *pvParameters) {
  ...
  if (countForStandBy <= standByTime && touchRead(touchPin)>=40) {
    countForStandBy  ++;
    ...
    }else if (countForStandBy >= standByTime && touchRead(touchPin)>=40 && countForStatusAnimation >= 260 && countForClockAnimation2 <= 7970){ //countForClockAnimation2 value = 5 minutes
      showTime();
